...
<html>
    ...
    <head>
		...
		<link href="{{ASSET_URL}}admin/css/bootstrap-responsive.min.css" rel="stylesheet" />
                        ...
                        <h3 class="header blue lighter smaller span12" style="margin-top:0px;margin-bottom:7px;">
                            <div class="row-fluid dataTables_wrapper span12">
                                <div class="text-center">
									...
									<a data-toggle="modal" class="btn btn-primary" href="{{URL::route('admin.core.system.student.add')}}">页面添加窗口</a>
                                    <a href="javascript:;" onclick="add();" class="btn btn-outline btn-default">添加</a>
									<a data-toggle="modal" class="btn btn-primary" href="#modal-form">打开添加窗口</a>
...
<!--                                <button class="btn btn-small btn-primary"><i class="icon-ok bigger-110"></i>审核</button>
        ...
        <div class="form-group"><label>年龄</label> <input type="text" name='age' placeholder="请输入您的年龄" class="form-control"></div>
                                <div>
									...
									<button class="btn btn-white" id='regsub' type="submit">确定</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
<a class="btn btn-primary btn-rounded"  href="javascript:void(0);" title="页面修改" onclick="tvedits({id});">\
<i class="icon-zoom-in bigger-130"></i>\
</a>\
<a class="btn btn-white btn-bitbucket"  href="javascript:void(0);" title="弹窗修改" onclick="tvedit({id});">\
<i class="icon-pencil bigger-130"></i>\
</a>\
<a class="btn btn-primary dim" href="javascript:void(0);" onclick="datadel({id});">\
...
<i class="icon-qrcode bigger-120"></i>\
        ...
        <script type="text/javascript">
			...
			$('body').on('click', '#regsub', function(){
				var obj = $('#wt-form');
				alert(obj.serialize())
				var actionUrl = "{{URL::route('admin.core.system.student.insert')}}";
				 $.ajax({
					type:'get',
                    url:'{{URL::route('admin.core.system.student.insert')}}',
                    data:obj.serialize(),
                    //cache:false,
                    success:function(data){
                    // 验证不通过
...
// 删除
    ...
    {
		var obj = 'listid=' + id;
			var actionUrl = "{{URL::route('admin.core.system.student.del')}}";
            $.ajax({
                type: 'get',
                url: actionUrl,
                data: obj,
	...
	function tvedit(id){
		layer.open({
			...
			shadeClose: true, //点击遮罩关闭层
			area : ['800px' , '520px'],
			content: ['{{URL::route('admin.core.system.student.edit')}}?id='+id,'no'],
		});
	}

	//页面修改
	function tvedits(id){
		var actionUrl = "{{URL::route('admin.core.system.student.edit')}}?id="+id;
		...
		laySum = layer.open({
					...
					shadeClose: true, //点击遮罩关闭层
					area : ['100%' , '100%'],
					content: ["{{URL::route('admin.core.system.student.edit1')}}?id="+id,'no'],
					style: 'width:100%; height:'+ document.documentElement.clientHeight +'px; background-color:#F2F2F2; border:none;'
				});
	
		// $editUrl = "{{URL::route('admin.core.system.student.edit')}}?id="+id;
